...
/**
 * Sign transaction using Bowhead node.
 */
public class SignTransactionIT extends Scenario {
    ...
    public void testSignTransaction() throws Exception {
        ...
        AhtSign ahtSign = web3j.ahtSign(ALICE.getAddress(), Numeric.toHexString(hashed))
                .sendAsync().get();

        String signature = ahtSign.getSignature();
        assertNotNull(signature);
        assertFalse(signature.isEmpty());
    }

    private static RawTransaction createTransaction() {
        BigInteger value = Convert.toCell("1", Convert.Unit.AHT).toBigInteger();
