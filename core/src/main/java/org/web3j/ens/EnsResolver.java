...
public class EnsResolver {
    ...
    public String resolve(String contractId) {
        if (isValidEnsName(contractId)) {
            ...
            try {
                ...
                throw new RuntimeException("Unable to execute Bowhead request", e);
            }
     ...
     * @param address an bowhead address, example: "0x314159265dd8dbb310642f98f50c066173c1259b"
    ...
    boolean isSynced() throws Exception {
        ...
        if (ahtSyncing.isSyncing()) {
            return false;
        } else {
            AhtBlock ahtBlock =
                    web3j.ahtGetBlockByNumber(DefaultBlockParameterName.LATEST, false).send();
            long timestamp = ahtBlock.getBlock().getTimestamp().longValue() * 1000;
